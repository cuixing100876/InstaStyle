...
with demo:
    ...
    with gr.Row():
        image = gr.Image(label="Image", scale=0.7,value='./style-images/haunted house/1 (1).jpg')
        with gr.Column():
            sty_init = gr.Textbox(label="Style",value='dark')
    ...
    with gr.Row():
            ...
            with gr.Column():
                with gr.Accordion(label="Training Options", open=False):
                    freeze_model =gr.Dropdown(choices=['crossattn_kv', 'crossattn'],label="Freeze model. ", value="crossattn_kv")#gr.Dropdown(["crossattn_kv", "crossattn"], label="freeze_model",value=["crossattn_kv"])
